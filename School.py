    ...
    try:
        ...
        if op.type == 13:
            ...
            if clientMid in op.param3:
                ...
                sendMention(op.param1, "Halo @!, Terimakasih Telah Mengundang Saya ke grup! :3")

        if op.type in [22, 24]:
            print ("[ 22 And 24 ] NOTIFIED INVITE INTO ROOM & NOTIFIED LEAVE ROOM")
            if settings["autoLeave"] == True:
                sendMention(op.param1, "Kakak Jahat")
...
# Pembatas Script #
                                ...
                                if msg.to in read["readPoint"]:
                                    ...
                                    client.sendMessage(msg.to, "Lurking belum diaktifkan, aktifkan lurking terlebih dahulu")
...
# Pembatas Script #   
                                ...
                                if len(cond) == 1:
                                    ...
                                    ret_ += "\n\nUntuk Melihat Details Music, silahkan gunakan command {}Musik {}|「number」".format(str(setKey), str(search))
                                    client.sendMessage(to, str(ret_))
                                elif len(cond) == 2:
                                    num = int(cond[1])
                                ...
                                if len(cond) == 1:
                                    ...
                                    ret_ += "\n\nUntuk Melihat Details Lyric, silahkan gunakan command {}Lirik {}|「number」".format(str(setKey), str(search))
                                    client.sendMessage(to, str(ret_))
                                elif len(cond) == 2:
                                    num = int(cond[1])
